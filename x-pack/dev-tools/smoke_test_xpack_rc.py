...
def java_exe():
  path = JAVA_HOME
  return f'export JAVA_HOME="{path}" PATH="{path}/bin:$PATH" JAVACMD="{path}/bin/java"'
...
def smoke_test_release(release, files, release_hash):
  for release_file in files:
    ...
    run(f'{java_exe()}; ES_JAVA_OPTS="-Des.plugins.staging={release_hash}" {es_plugin_path} install -b x-pack'
        )
    headers = {
        'Authorization':
...
if __name__ == "__main__":
  ...
  if url:
    ...
    download_url = f'https://staging.elastic.co/{version}-{hash}/downloads/elasticsearch/elasticsearch-{version}.zip'
